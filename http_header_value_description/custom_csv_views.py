...
def export_csv(request):
    ...
    csv_data = []
    for item in items:
        row = [item.name, item.value, item.description]
        csv_data.append(row)
    ...
    template = loader.get_template('http_header_value_description_template.txt')
    c = {'data': csv_data}
...
def import_csv(request):
    ...
    for line in f:
        ...
        line = line.split(':')
        hhd = HttpHeaderValueDescription()
        # Parse header name
        hhd.name = line[0].replace('"', '')
        # Parse header value and description from line[1]
        hhd_vd = line[1].split('" "')
        hhd.value = hhd_vd[0].replace('"', '').lstrip()
        hhd.description = hhd_vd[1].replace('"', '').lstrip()
        record = HttpHeaderValueDescription.objects.filter(name=hhd.name).first()
        if record == None:
            hhd.save()
