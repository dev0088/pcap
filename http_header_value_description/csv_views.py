...
from .resources import HttpHeaderValueDescriptionResource

def export_csv(request):
    http_header_value_description_resource = HttpHeaderValueDescriptionResource()
    dataset = http_header_value_description_resource.export_with_custom_delimiter()

    t = datetime.datetime.now()
    t = '{:%Y-%m-%d_%H-%M}'.format(t)
    
    response = HttpResponse(dataset, content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename="http_header_value_descriptions-{str_date}.csv"'.format(str_date=t)
...
def import_csv(request):
    ...
    dataset = tablib.import_set(
        new_http_header_value_descriptions,
        format='csv',
    ...
    http_header_value_description_resource = HttpHeaderValueDescriptionResource()
    # Test import now
    result = http_header_value_description_resource.import_data(dataset, dry_run=True)
    # Remove temp file
    os.remove(stored_path)

    if not result.has_errors():
        # Actually import now
        http_header_value_description_resource.import_data(dataset, dry_run=False)
...
def export_json(request):
    http_header_value_description_resource = HttpHeaderValueDescriptionResource()
    dataset = http_header_value_description_resource.export()
    response = HttpResponse(dataset.json, content_type='application/json')
    response['Content-Disposition'] = 'attachment; filename="http_header_value_descriptions.json"'
    return response
...
def export_excel(request):
    ...
    response['Content-Disposition'] = 'attachment; filename="http_header_value_descriptions.xls"'
    return response
